...
class MusicViewModel(
    ...
    /** Inserts [track] to play right after the current track (3-dot menu -> "Add to next"). */
    fun addToQueueNext(track: Track) {
        val queue = _playQueue.value.toMutableList()
        queue.removeAll { it.id == track.id && it.id != _currentTrack.value?.id }
        val insertAt = (_currentQueueIndex.value + 1).coerceIn(0, queue.size)
        queue.add(insertAt, track)
        _playQueue.value = queue
    }

    /** Appends [track] to the end of the queue (3-dot menu -> "Add to queue"). */
    ...
    /**
     * Temporarily fades the active player's volume - used while dragging the
     * mini player downward, so the audio audibly fades out as the bar
     * follows the finger. 1f = full volume (bar at rest), 0f = fully dragged
     * off screen.
     */
    fun setPlaybackVolume(level: Float) {
        try {
            activePlayer()?.volume = level.coerceIn(0f, 1f)
        } catch (e: Exception) {
            // Some Player implementations (e.g. certain Cast sessions) don't
            // support direct volume control - safe to ignore, the drag
            // animation itself still communicates the dismissal.
        }
    }

    /**
     * Fully stops playback and clears the current track so the mini player
     * disappears entirely. Used when the mini player is dragged down past
    ...
    fun stopAndClearPlayback() {
        try {
            activePlayer()?.apply {
                pause()
                stop()
                volume = 1f // restore for the next track
            }
        } catch (e: Exception) {
